  ...
  return new Promise((resolve, reject) => {
    ...
    child_process.exec(
      ...
      (fetchError, fetchStdout, fetchStderr) => {
        ...
        if (fetchError) {
          console.warn(
            'Fetch error (branch might not exist on remote):',
            fetchError.message
          );
        }
        ...
        for (const block of commitBlocks) {
          ...
          for (let i = lines.length - 2; i >= 0; i--) {
            ...
            const dateMatch = line.match(
              /\|\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+[+-]\d{4})\s*$/
            );
            if (dateMatch) {
              dateLineIndex = i;
        ...
        if (error) {
          ...
          child_process.exec(
            ...
            () => {
              ...
              reject(
                new Error(
                  `Failed to cherry-pick commit ${commitId}: ${error.message}`
                )
              );
            }
          );
    ...
    child_process.exec(
      ...
      (headError, headStdout) => {
        ...
        if (needCheckout && checkoutBranch) {
          ...
          child_process.exec(
            ...
            checkoutError => {
              if (checkoutError) {
                reject(
                  new Error(
                    `Failed to checkout ${checkoutBranch} before deleting branch: ${checkoutError.message}`
                  )
                );
                return;
              }
          ...
          reject(
            new Error(
              `Cannot delete branch ${branchName} because it is currently checked out. Please provide a branch to checkout first.`
            )
          );
        } else {
