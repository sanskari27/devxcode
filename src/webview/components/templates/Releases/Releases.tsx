...
export const Releases: React.FC = () => {
    const { releases, createRelease, deleteRelease, addChangeToRelease, updateChangeInRelease, getChangeProgress, getReleaseProgressSync } = useReleasesService();
    ...
    return (
        <Section
            title='Releases'
            action={handleAddRelease}
            actionIcon={Plus}
            defaultExpanded
        >
            {releases.map((release) => {
                const releaseProgress = releaseProgressMap.get(release.id) ?? { percentage: 0, totalItems: 0, completedItems: 0 };
                    ...
                    >
                        ...
                        >
                            {release.changes.map((change, index) => {
                                const changeProgress = changeProgressMap.get(change.id) ?? { completed: 0, total: 0 };
                                const changeProgressPercentage = Math.round((changeProgress.completed / changeProgress.total) * 100);
                                return (
                                    <Section
                                        key={change.id}
